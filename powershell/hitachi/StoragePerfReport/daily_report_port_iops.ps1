...
$HTnM_XML = "D:\scripts\XMLCONFIG\daily_port_iops.xml"
...
$timeepoch = import-csv ($CSVDirectory + $cleanCSV) | select @{name="EpochTime";e={(New-TimeSpan -Start $epoch -End $_."Date and Time").TotalSeconds}}, "Port Name", "Avg I/O /sec", "Max I/O /sec"
...
foreach ($Port in $Ports){ 
    ...
    $args_create2 += ("DS:Usage_" + $Port + ":GAUGE:300:0:1000000 RRA:AVERAGE:0.5:1:9600" + " ")
}
...
1..(($portData[0]).count - 1) | foreach {
    ...
    $args_update3 = ""
    foreach ($n in $num) {
        $args_update3 += ($portData[$n][$_]."Max I/O /sec" + ":")       
    }
